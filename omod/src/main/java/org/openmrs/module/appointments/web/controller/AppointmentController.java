...
public class AppointmentController extends BaseRestController {
    ...
    public ResponseEntity<AppointmentDefaultResponse> getAppointmentByUuid(@RequestParam(value = "uuid") String uuid)  {
        ...
        if(appointment == null) {
            // An unknown uuid is a 404, not a server fault - throwing produced a 500 with a full
            // stack trace on every miss. Absent and not-visible are deliberately one case: telling
            // them apart would disclose that the appointment exists somewhere else.
